...
Se me borró la práctica no sé cómo así que la he tenido que repetir entera.
...
```{r}
library(phangorn)
mtdna <- read.phyDat("/home/asanloa/bioinformatica/data/31-03/mtDNA.fasta", format = "fasta")
```
...
Primero se comprueba que tenemos le objeto mtdna. Después la salida dice que tenemos 42 secuencias con 16603 posiciones nucleotídicas.
...
- ¿De qué clase son los objetos `mt_nj` y `mt_upgma`? Son árboles filogenéticos.
...
add.scale.bar() añade una barra de escala con distancia evolutiva y sirve para interpretar las longitudes de rama mientras qie axisPhylo() añade un eje temporal o de distancia en arboles enraizados, lo cual es útil cuando las ramas representan tiempo.
...
- ¿Se corresponden bien las distancias evolutivas estimadas con las dataciones?
  ...
  No, hay ramas que no se corresponen con las fechas, por lo que deberían haberse quedado más hacia atrás.
...
Se puede asumir que las dataciones moleculares no son fiables o que las muestras arqueológicas sean erróneas. También puede deberse a un error estadístico a la hora de corregir los errores de la datación, generándo un alargamiento en las ramas incorrecto.
...
- ¿Cuántas veces mayor es la distancia entre humanos y chimpancés que entre humanos y neandertales?
  ...
  ```{r}
  d_hn <- mean(distMat[modernos, neandertales])
  d_hc <- mean(distMat[modernos, chimpances])
...
#añado esto aquí para posteriores ejecuciones
...
```{r}
modelos_homo <- modelTest(mtdna28,
                          control = pml.control(trace = 0))

modelos_homo[which.min(modelos_homo$BIC), ]
```
...
mt_ml_homo <- pml_bb(modelos_homo,
                     control = pml.control(trace = 0))
